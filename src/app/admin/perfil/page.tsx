...
import { Mail, Calendar, Shield, User, Pencil, Check, X, Loader2, Camera } from "lucide-react";
...
export default function PerfilAdminPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setFirstName(capitalized.split(" ")[0] || "Admin");
      setEmail(user.email ?? "");
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 5 * 1024 * 1024) { setSaveMsg({ type: "error", text: "La imagen no puede superar los 5 MB." }); return; }
    ...
    if (uploadError) { setSaveMsg({ type: "error", text: "Error al subir la foto." }); setUploadingPhoto(false); return; }
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1 text-sm">Información de tu cuenta de administrador.</p>
      </div>
          ...
          <div className="relative flex items-center gap-4">
            ...
            >
              {avatar
                ...
                : <div className="w-full h-full flex items-center justify-center bg-white/20"><span className="text-white text-2xl font-bold">{firstName.charAt(0)}</span></div>
              }
            ...
            <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="hidden" onChange={handlePhotoChange} />
            <div>
        ...
        <div className="px-6 pb-6 pt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {[
              ...
              { icon: Shield,   label: "Rol en la plataforma", value: "Administrador" },
      ...
      <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-orange-800 mb-0.5">Cuenta de administrador</p>
          <p className="text-xs text-orange-600 leading-relaxed">
            Tienes acceso completo a la plataforma. Gestiona usuarios, cursos y configuración desde el panel lateral.
          </p>
        </div>
      </div>
    </div>
